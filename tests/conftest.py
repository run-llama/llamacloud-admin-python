...
from llama_cloud_admin import LlamaCloudAdmin, DefaultAioHttpClient, AsyncLlamaCloudAdmin

from llama_cloud_admin._utils import is_dict
...
logging.getLogger("llama_cloud_admin").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[LlamaCloudAdmin]:
    strict = getattr(request, 'param', True)
    if not isinstance(strict, bool):
      raise TypeError(f'Unexpected fixture parameter type {type(strict)}, expected {bool}')

    with LlamaCloudAdmin(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client :
        yield client

@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncLlamaCloudAdmin]:
    ...
    async with AsyncLlamaCloudAdmin(base_url=base_url, api_key=api_key, _strict_response_validation=strict, http_client=http_client) as client :
        yield client